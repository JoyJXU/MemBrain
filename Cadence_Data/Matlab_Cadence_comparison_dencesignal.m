...
close all;
%% load cadence data
load zi_modified_intence
load zj_modified_intence
load pi_modified_intence
load pj_modified_intence
load pij_modified_intence
load logpi_modified_intence
load logpj_modified_intence
load logpij_modified_intence
load wij_modified_intence
...
%% matlab comparison
% Matlab comparison of 5s/1s dence signal
...
title('Matlab comparison of 5s dence signal ')
...
subplot(511),
x=1:points;
plot(y*0.001,si(1:1000),'Color',blue1, 'linewidth',0.8);hold on
plot(y*0.001,sj(1:1000),'Color',green1, 'linewidth',0.8);
title('Matlab comparison of 1s dence signal ')
subplot(512),

plot(y*0.001,zi(1:1000),'Color',blue2, 'linewidth',1.5);hold on
plot(y*0.001,New_Z_i_shift(1:1000),'Color',blue1, 'linewidth',1.5);hold on
plot(y*0.001,zj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,New_Z_j_shift(1:1000),'Color',green1, 'linewidth',1.5);

subplot(513);
plot(y*0.001,pi(1:1000),'Color',blue2, 'linewidth',1.5);hold on
plot(y*0.001,New_P_i_shift(1:1000),'Color',blue1, 'linewidth',1.5);hold on
plot(y*0.001,pj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,New_P_j_shift(1:1000),'Color',green1, 'linewidth',1.5);hold on
plot(y*0.001,pij(1:1000),'Color',orange2, 'linewidth',1.5);hold on
plot(y*0.001,New_P_ij_shift(1:1000),'Color',orange1, 'linewidth',1.5);

subplot(514);
plot(y*0.001,wij(1:1000),'Color',orange2, 'linewidth',1.5);hold on
plot(y*0.001,W_ij_shift(1:1000),'Color',orange1, 'linewidth',1.5);

subplot(515);
plot(y*0.001,bj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,b_j_shift(1:1000),'Color',green1, 'linewidth',1.5);

%% cadence comparison

% cadence data error analysis
Ca_Z_error_i = abs(zi_modified_intence - zi);
Ca_max_error_z_i = max(Ca_Z_error_i);
Ca_mean_error_z_i = mean(Ca_Z_error_i);
Ca_corr_zi = corrcoef(zi,zi_modified_intence);
Ca_corr_z_i = Ca_corr_zi(2,1);
Ca_rrmse_zi=sqrt((sum((zi_modified_intence - zi).^2)/(sum(zi.^2)))/5000);
Ca_rmse_zi=sqrt(mean((zi_modified_intence - zi).^2));


Ca_Z_error_j = abs(zj_modified_intence - zj);
Ca_max_error_z_j = max(Ca_Z_error_j);
Ca_mean_error_z_j = mean(Ca_Z_error_j);
Ca_corr_zj = corrcoef(zj,zj_modified_intence);
Ca_corr_z_j = Ca_corr_zj(2,1);
Ca_rrmse_zj=sqrt((sum((zj_modified_intence - zj).^2)/(sum(zj.^2)))/5000);
Ca_rmse_zj=sqrt(mean((zj_modified_intence - zj).^2));

Ca_P_error_i = abs(pi_modified_intence - pi);
Ca_max_error_p_i = max(Ca_P_error_i);
Ca_mean_error_p_i = mean(Ca_P_error_i);
Ca_corr_pi = corrcoef(pi,pi_modified_intence);
Ca_corr_p_i = Ca_corr_pi(2,1);
Ca_rrmse_pi=sqrt((sum((pi_modified_intence - pi).^2)/(sum(pi.^2)))/5000);
Ca_rmse_pi=sqrt(mean((pi_modified_intence - pi).^2));


Ca_P_error_j = abs(pj_modified_intence - pj);
Ca_max_error_p_j = max(Ca_P_error_j);
Ca_mean_error_p_j = mean(Ca_P_error_j);
Ca_corr_pj = corrcoef(pj,pj_modified_intence);
Ca_corr_p_j = Ca_corr_pj(2,1);
Ca_rrmse_pj=sqrt((sum((pj_modified_intence - pj).^2)/(sum(pj.^2)))/5000);
Ca_rmse_pj=sqrt(mean((pj_modified_intence - pj).^2));

Ca_P_error_ij = abs(pij_modified_intence - pij);
Ca_max_error_p_ij = max(Ca_P_error_ij);
Ca_mean_error_p_ij = mean(Ca_P_error_ij);
Ca_corr_pij = corrcoef(pij,pij_modified_intence);
Ca_corr_p_ij = Ca_corr_pij(2,1);
Ca_rrmse_pij= sqrt((sum((pij_modified_intence - pij).^2)/(sum(pij.^2)))/5000);
Ca_rmse_pij=sqrt(mean((pij_modified_intence - pij).^2));

Ca_W_error_ij = abs(wij_modified_intence - wij);
Ca_max_error_w_ij = max(Ca_W_error_ij);
Ca_mean_error_w_ij = mean(Ca_W_error_ij);
Ca_corr_wij = corrcoef(wij,wij_modified_intence);
Ca_corr_w_ij = Ca_corr_wij(2,1);
Ca_rrmse_wij= sqrt((sum((wij_modified_intence- wij).^2)/(sum(wij.^2)))/5000);
Ca_rmse_wij=sqrt(mean((wij_modified_intence- wij).^2));

logpj_modified_intence=logpj_modified_intence-4.60517;
Ca_b_error_j = abs(logpj_modified_intence - bj);
Ca_max_error_b_j = max(Ca_b_error_j);
Ca_mean_error_b_j = mean(Ca_b_error_j);
Ca_corr_bj = corrcoef(bj,logpj_modified_intence);
Ca_corr_b_j = Ca_corr_bj(2,1);
Ca_rrmse_bj= sqrt((sum((logpj_modified_intence - bj).^2)/(sum(bj.^2)))/5000);
Ca_rmse_bj=sqrt(mean((logpj_modified_intence - bj).^2));
...
% Cadence comparison of 5s/1s dence signal
figure(3)
subplot(511),
x=1:points;
plot(x*0.001,si,'Color',blue1, 'linewidth',0.8);hold on
plot(x*0.001,sj,'Color',green1, 'linewidth',0.8);
title('Cadence comparison of 5s dence signal ')

subplot(512),
plot(x*0.001,zi,'Color',blue2, 'linewidth',1.5);hold on
plot(x*0.001,zi_modified_intence,'Color',blue1, 'linewidth',1.5);hold on
plot(x*0.001,zj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,zj_modified_intence,'Color',green1, 'linewidth',1.5);

subplot(513);
plot(x*0.001,pi,'Color',blue2, 'linewidth',1.5);hold on
plot(x*0.001,pi_modified_intence,'Color',blue1, 'linewidth',1.5);hold on
plot(x*0.001,pj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,pj_modified_intence,'Color',green1, 'linewidth',1.5);hold on
plot(x*0.001,pij,'Color',orange2, 'linewidth',1.5);hold on
plot(x*0.001,pij_modified_intence,'Color',orange1, 'linewidth',1.5);

subplot(514);
plot(x*0.001,wij,'Color',orange2, 'linewidth',1.5);hold on
plot(x*0.001,wij_modified_intence,'Color',orange1, 'linewidth',1.5);

subplot(515);
plot(x*0.001,bj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,logpj_modified_intence,'Color',green1, 'linewidth',1.5);
...
plot(y*0.001,si(1:1000),'Color',blue1, 'linewidth',0.8);hold on
plot(y*0.001,sj(1:1000),'Color',green1, 'linewidth',0.8);
title('Cadence comparison of 1s dence signal ')

subplot(512),
plot(y*0.001,zi(1:1000),'Color',blue2, 'linewidth',1.5);hold on
plot(y*0.001,zi_modified_intence(1:1000),'Color',blue1, 'linewidth',1.5);hold on
plot(y*0.001,zj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,zj_modified_intence(1:1000),'Color',green1, 'linewidth',1.5);

subplot(513);
plot(y*0.001,pi(1:1000),'Color',blue2, 'linewidth',1.5);hold on
plot(y*0.001,pi_modified_intence(1:1000),'Color',blue1, 'linewidth',1.5);hold on
plot(y*0.001,pj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,pj_modified_intence(1:1000),'Color',green1, 'linewidth',1.5);hold on
plot(y*0.001,pij(1:1000),'Color',orange2, 'linewidth',1.5);hold on
plot(y*0.001,pij_modified_intence(1:1000),'Color',orange1, 'linewidth',1.5);

subplot(514);
plot(y*0.001,wij(1:1000),'Color',orange2, 'linewidth',1.5);hold on
plot(y*0.001,wij_modified_intence(1:1000),'Color',orange1, 'linewidth',1.5);

subplot(515);
plot(y*0.001,bj(1:1000),'Color',green2, 'linewidth',1.5);hold on
plot(y*0.001,logpj_modified_intence(1:1000),'Color',green1, 'linewidth',1.5);