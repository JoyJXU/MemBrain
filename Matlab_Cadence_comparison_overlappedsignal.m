...
Ca_Z_error_i = abs(zi_nopara - zi);
Ca_max_error_z_i = max(Ca_Z_error_i);
Ca_mean_error_z_i = mean(Ca_Z_error_i);
Ca_corr_zi = corrcoef(zi,zi_nopara);
Ca_corr_z_i = Ca_corr_zi(2,1);
Ca_rrmse_zi= sqrt((sum((zi_nopara - zi).^2)/(sum(zi.^2)))/5000);
Ca_rmse_zi=sqrt(mean((zi_nopara - zi).^2));

Ca_Z_error_j = abs(zj_nopara - zj);
Ca_max_error_z_j = max(Ca_Z_error_j);
Ca_mean_error_z_j = mean(Ca_Z_error_j);
Ca_corr_zj = corrcoef(zj,zj_nopara);
Ca_corr_z_j = Ca_corr_zj(2,1);
Ca_rrmse_zj= sqrt((sum((zj_nopara -zj).^2)/(sum(zj.^2)))/1000);
Ca_rmse_zj=sqrt(mean((zj_nopara - zj).^2));

Ca_P_error_i = abs(pi_nopara - pi);
Ca_max_error_p_i = max(Ca_P_error_i);
Ca_mean_error_p_i = mean(Ca_P_error_i);
Ca_corr_pi = corrcoef(pi,pi_nopara);
Ca_corr_p_i = Ca_corr_pi(2,1);
Ca_rrmse_pi= sqrt((sum((pi_nopara - pi).^2)/(sum(pi.^2)))/1000);
Ca_rmse_pi=sqrt(mean((pi_nopara - pi).^2));

Ca_P_error_j = abs(pj_nopara - pj);
Ca_max_error_p_j = max(Ca_P_error_j);
Ca_mean_error_p_j = mean(Ca_P_error_j);
Ca_corr_pj = corrcoef(pj,pj_nopara);
Ca_corr_p_j = Ca_corr_pj(2,1);
Ca_rrmse_pj= sqrt((sum((pj_nopara - pj).^2)/(sum(pj.^2)))/1000);
Ca_rmse_pj=sqrt(mean((pj_nopara - pj).^2));

Ca_P_error_ij = abs(pij_nopara - pij);
Ca_max_error_p_ij = max(Ca_P_error_ij);
Ca_mean_error_p_ij = mean(Ca_P_error_ij);
Ca_corr_pij = corrcoef(pij,pij_nopara);
Ca_corr_p_ij = Ca_corr_pij(2,1);
Ca_rrmse_pij= sqrt((sum((pij_nopara - pij).^2)/(sum(pij.^2)))/1000);
Ca_rmse_pij=sqrt(mean((pij_nopara - pij).^2));

Ca_W_error_ij = abs(wij_nopara - wij);
Ca_max_error_w_ij = max(Ca_W_error_ij);
Ca_mean_error_w_ij = mean(Ca_W_error_ij);
Ca_corr_wij = corrcoef(wij,wij_nopara);
Ca_corr_w_ij = Ca_corr_wij(2,1);
Ca_rrmse_wij= sqrt((sum((wij_nopara - wij).^2)/(sum(wij.^2)))/1000);
Ca_rmse_wij=sqrt(mean((wij_nopara- wij).^2));

logpj_nopara=logpj_nopara-4.60517;
Ca_b_error_j = abs(logpj_nopara - bj);
Ca_max_error_b_j = max(Ca_b_error_j);
Ca_mean_error_b_j = mean(Ca_b_error_j);
Ca_corr_bj = corrcoef(bj,logpj_nopara);
Ca_corr_b_j = Ca_corr_bj(2,1);
Ca_rrmse_bj= sqrt((sum((logpj_nopara - bj).^2)/(sum(bj.^2)))/1000);
Ca_rmse_bj=sqrt(mean((logpj_nopara - bj).^2));
...
% Cadence comparison of overlapping signals
figure(2)
subplot(511),
x=1:points;
plot(x*0.001,si,'Color',blue1, 'linewidth',0.8);hold on
plot(x*0.001,sj,'Color',green1, 'linewidth',0.8);
title('Cadence comparison of overlapping signals')

subplot(512),
plot(x*0.001,zi,'Color',blue2, 'linewidth',1.5);hold on
plot(x*0.001,zi_nopara,'Color',blue1, 'linewidth',1.5);hold on
plot(x*0.001,zj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,zj_nopara,'Color',green1, 'linewidth',1.5);

subplot(513);
plot(x*0.001,pi,'Color',blue2, 'linewidth',1.5);hold on
plot(x*0.001,pi_nopara,'Color',blue1, 'linewidth',1.5);hold on
plot(x*0.001,pj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,pj_nopara,'Color',green1, 'linewidth',1.5);hold on
plot(x*0.001,pij,'Color',orange2, 'linewidth',1.5);hold on
plot(x*0.001,pij_nopara,'Color',orange1, 'linewidth',1.5);

subplot(514);
plot(x*0.001,wij,'Color',orange2, 'linewidth',1.5);hold on
plot(x*0.001,wij_nopara,'Color',orange1, 'linewidth',1.5);

subplot(515);
plot(x*0.001,bj,'Color',green2, 'linewidth',1.5);hold on
plot(x*0.001,logpj_nopara,'Color',green1, 'linewidth',1.5);
